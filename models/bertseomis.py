...
class BERT_SEOMISModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout):
        super(BERT_SEOMISModel, self).__init__()
    ...
    def forward(self, **kwargs):
        ...
        return torch.sigmoid(0.83 * bias_pred + 0.15 * event_prob  + 0.02 * soc_prob), torch.sigmoid(event_prob), torch.sigmoid(soc_prob), torch.sigmoid(bias_pred)  # 1.30-0.27-0.03 torch.sigmoid(1.30 * bias_pred - 0.27 * entity_prob - 0.03 * soc_prob)
...
class Trainer():
    ...
    def train(self, logger = None):
        ...
        self.model = BERT_SEOMISModel(self.config['emb_dim'], self.config['model']['mlp']['dims'], self.config['model']['mlp']['dropout'])
        ...
        for epoch in range(self.config['epoch']):
            ...
            if mark == 'save'or epoch == (self.config['epoch']-1):
                torch.save(self.model.state_dict(),
                    os.path.join(self.save_path, 'parameter_bertseomis.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_path, 'parameter_bertseomis.pkl')))
        ...
        return future_results, os.path.join(self.save_path, 'parameter_bertseomis.pkl')
